      ...
      <nav className="fixed top-0 left-0 w-full h-auto bg-gray-800 z-10 flex items-center justify-around">
        ...
        <ul className={active ? "nav-ul flex gap-11 md:gap-14 xs:gap-12 lg:basis-3/6 md:basis-4/6 md:justify-end active" : " nav-ul flex gap-14 lg:basis-3/6 md:basis-4/6 justify-end"}>
          ...
          <li className="dropdown-li">
            <Link className="no-underline font-semibold flex-center gap-2" onClick={() => {setCategoryDropdown(!categoryDropdown); setCountryDropdown(false)}}>
              Top Headlines
              <FontAwesomeIcon className={categoryDropdown ? "down-arrow-icon down-arrow-icon-active" : "down-arrow-icon"} icon={faCircleArrowDown} />
            </Link>
            <ul className={categoryDropdown ? "dropdown p-2 show-dropdown" : "dropdown p-2"}>
                {categories.map((element, index) => {
                  return (
                    <li key={index} onClick={() => { setCategoryDropdown(!categoryDropdown) }}>
          ...
          <li className="dropdown-li">
            <Link className="no-underline font-semibold flex items-center gap-2" onClick={() => { setCountryDropdown(!countryDropdown); setCategoryDropdown(false)}}>
            Country 
            <FontAwesomeIcon className={countryDropdown ? "down-arrow-icon down-arrow-icon-active" : "down-arrow-icon"} icon={faCircleArrowDown} /></Link>
            <ul className={countryDropdown ? "dropdown p-2 show-dropdown" : "dropdown p-2"}>
              {countries.map((element, index) => {
              return ( 
                  <li key={index} onClick={() => setCountryDropdown(!countryDropdown)}>
